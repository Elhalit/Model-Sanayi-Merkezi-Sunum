...
const floorPlanImages = {
  ...
  G: 'https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?ixlib=rb-4.0.3&auto=format&fit=crop&w=1400&h=900',
  H: 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?ixlib=rb-4.0.3&auto=format&fit=crop&w=1400&h=900'
} as const;

type FilterType = 'all' | 'available' | 'sold' | 'reserved' | 'with-firms';

export default function FloorPlansSection2() {
  const [activeBlock, setActiveBlock] = useState<string>('F');
  ...
  useEffect(() => {
    const loadFloorPlanData = async () => {
      try {
        // Load units data
        const unitsResponse = await fetch('/2.etab - 2.etab.csv');
        const unitsContent = await unitsResponse.text();
        let parsedUnits = parseCSV(unitsContent, '2');
  ...
  const handleDownload = () => {
    ...
    link.href = floorPlanImages[activeBlock as keyof typeof floorPlanImages] || floorPlanImages.F;
    link.download = `${activeBlock}-blok-kat-plani-2etap.jpg`;
  ...
  const filteredUnits = currentBlockUnits.filter(unit => {
    ...
    if (filter === 'all') {
      ...
      matchesFilter = getFirmInfoForUnit(firms, unit.block, unit.unitNumber, '2') !== null;
    } else {
  ...
  return (
    <section
      ...
      data-testid="floorplans-section-2"
    >
              ...
              {selectedUnit ? (
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Firm Information */}
                    {(() => {
                      const firmInfo = getFirmInfoForUnit(firms, selectedUnit.block, selectedUnit.unitNumber, '2');
                      return firmInfo ? (
              ...
              >
                {sortedUnits.map((unit) => {
                  ...
                  if (unitNum <= 18) {
                    ...
                    if (totalBottom === 4) {
                      // 4 units (F Block mostly): 1, 3, 5, 7 
                      ...
                      // 3 units: Adapted for 8 cols (3-2-3)
                      // 19->1(span3), 20->4(span2), 21->6(span3)
                      if (bottomIndex === 0) { colStart = 1; colSpan = 3; }
                      else if (bottomIndex === 1) { colStart = 4; colSpan = 2; }
                      else { colStart = 6; colSpan = 3; }
                    } else {
                      // Default/Fallback
                      if (totalBottom === 1) {
                        colStart = 3;
                        ...
                        // Fallback logic
                        colSpan = Math.max(1, Math.floor(8 / totalBottom));
